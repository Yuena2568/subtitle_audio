...
MAX_RETRY = 10  # 每个 P 最大重试次数
# B站 cookies 文件：默认为项目根目录下的 cookies.txt
# 可以通过环境变量 BILIBILI_COOKIES_FILE 自定义
...
def merge_subtitle_jsons(bv_dir, mp4_files):
    ...
    if merged_data:
        ...
        print(f"多 P 字幕已合并成总 JSON: {merged_json_path}")
    return merged_json_path

# ================================
# 多 P 视频无重新编码合并
# ================================
def concat_p_videos(bv_dir, mp4_files):
    if not mp4_files:
...
def download_bilibili_video(bv_id):
    ...
    # 获取视频信息
    ydl_opts_info = {'quiet': True, 'skip_download': True, 'cookiefile': COOKIES_FILE}
    with yt_dlp.YoutubeDL(ydl_opts_info) as ydl:
        info = ydl.extract_info(url, download=False)

    p_list = info.get('entries', None)
    if p_list is None:
        p_list = [info]

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    folder_name = f"{info.get('title')}_{timestamp}"
    bv_dir = os.path.join(DOWNLOAD_ROOT, folder_name)
    os.makedirs(bv_dir, exist_ok=True)

    print(f"视频标题：{info.get('title')}")
    print(f"共 {len(p_list)} 个 P")
    print(f"保存路径：{bv_dir}\n")

    mp4_files = []
    failed_p_list = []

    for idx, p_info in enumerate(p_list, start=1):
        p_title = p_info.get('title', f"p{idx}")
        safe_title = f"{idx:02d}_{p_title}"
        ...
        for attempt in range(1, MAX_RETRY + 1):
            try:
                print(f"开始下载 P{idx}: {p_title} （尝试 {attempt}/{MAX_RETRY}）")
                with yt_dlp.YoutubeDL(ydl_opts_download) as ydl:
                    ydl.download([p_info['webpage_url']])
                ...
                if GENERATE_JSON:
                    srt_path = outtmpl.replace('%(ext)s', 'zh.srt')
                ...
                print(f"P{idx} 下载失败: {e}")
                time.sleep(2)

        if not success:
            failed_p_list.append(f"P{idx}: {p_title} （可能是 AV1 或其他问题）")

    # 保存失败记录
    if failed_p_list:
        failed_file = os.path.join(bv_dir, "failed_p.txt")
        with open(failed_file, 'w', encoding='utf-8') as f:
            for line in failed_p_list:
                f.write(line + "\n")
        print(f"失败的 P 已记录在 {failed_file}")

    # 合并视频（无重新编码）
    merged_video_path = concat_p_videos(bv_dir, mp4_files)
...
if __name__ == "__main__":
    ...
    bv_input = input("请输入 B 站 BV 号（例如 BV17zbzzcEUX）：").strip()
    download_bilibili_video(bv_input)
